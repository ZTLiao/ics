...
static void CheckHInfo(void) {
	...
	*/
	...
	uint64 partialmd5 = 0;
	for (x = 0; x < 8; x++) {
		uint64 temp = iNESCart.MD5[15 - x];
		partialmd5 |= (uint64) (temp << (x * 8));
		//printf("partialmd5 = %d\n", partialmd5);
    }
	CheckBad(partialmd5);
...
int iNESLoad(const char *name, FCEUFILE *fp, int OverwriteVidMode) {
	...
	if (FCEU_fread(&head, 1, 16, fp) != 16 || memcmp(&head, "NES", 3))
		return 0;
	head.cleanup();
	...
	printf("iNESLoad LoadedRomFName = %s\n", LoadedRomFName);
